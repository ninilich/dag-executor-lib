...
/** A class representing a Directed Acyclic Graph (DAG) for managing and executing getTasks with dependencies. It
  ...
  * @param mdcKey
  *   Key for Mapped Diagnostic Context (MDC). DAG will put the name of the task into MDC using this key. It could be
  *   later included into log messages for tracing and debugging and also allows to see which task was being executed
  *   when a particular log message was generated by logging frameworks like Log4j and SLF4J. This is particularly
  *   useful in multithreaded environments where different threads may be executing different tasks simultaneously. By
  *   default, is `DagTaskName`
  */
class DAG[T](maxThreads: Int = 0, awaitTime: Duration = Duration.Inf, mdcKey: String = "DagTaskName") {
  ...
  case class Node(task: RunnableDAGTask[T], name: TaskName, dependencies: List[Node] = List())

  /** A dummy task. This class is primarily used as a placeholder or for testing scenarios.
    */
  private class DummyTask extends RunnableDAGTask[T] {
  ...
  /** Adds a new task node with dependencies to the DAG.
    ...
    */
  private def addNode(task: RunnableDAGTask[T], name: TaskName, dependencies: List[Node]): Node = {
  ...
  /** Adds a task to the DAG by specifying the task name, the task itself, and its dependencies by their names.
    ...
    */
  def addTask(name: TaskName, task: RunnableDAGTask[T], dependencyNames: List[TaskName] = List.empty): Node = {
  ...
  private def executeTaskWithErrorHandling(node: Node): TaskExecutionResult[T] = {
    ...
    if (isExecuteTask) {
      try {
        MDC.put(mdcKey, s"${node.name}") // Set the name of the task in MDC
        val startTime = Instant.now() // Start timing
        val taskResult = node.task.run() // Execute the task
        val endTime = Instant.now() // End timing
        val duration = java.time.Duration.between(startTime, endTime).getSeconds
